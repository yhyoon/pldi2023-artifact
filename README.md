...
As an alternative to the VM image, we also offer a Docker Image (pldi2023_artifact_23_docker_img_x64_v2.tar.gz) for your convenience.
...
If you're operating in an ARM64 environment but still want to achieve optimal performance from abs_synth without having to run `probe`, an alternative Image is available (pldi2023_artifact_23_docker_img_arm64_v2.tar.gz). This image allows you to use the ./artifact commands without additional configuration, but please note that the results generated by `probe` may not be correct.
