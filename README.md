...
As an alternative to the VM image, we also offer a Docker Image (https://zenodo.org/record/7783701/files/pldi2023_artifact_23_docker_img_x64_v2.tar.gz?download=1) for your convenience.
Install [Docker Desktop](https://www.docker.com/products/docker-desktop/), download the gzipped docker image file and run with:
```sh
$ gzip -d pldi2023_artifact_23_docker_img_x64_v2.tar.gz
$ docker load -i pldi2023_artifact_23_docker_img_x64_v2.tar
# time-consuming process...

# if you are in Intel x86_64 environment
$ docker run -it pldi2023-artifact-23:v2
...
$ docker run -it --platform=linux/amd64 pldi2023-artifact-23:v2
(in-container) $ ./artifact batch ...
```

If you're operating in an ARM64 environment but still want to achieve optimal performance from abs_synth without having to run `probe`, an alternative Image is available (https://zenodo.org/record/7783701/files/pldi2023_artifact_23_docker_img_arm64_v2.tar.gz?download=1). This image allows you to use the ./artifact commands without additional configuration, but please note that the results generated by `probe` may not be correct.
